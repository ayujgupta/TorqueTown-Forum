...
function ArticleEditor() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      console.log("DRAFT"+localStorage.getItem("formDraft"+userInfo.id));
      if(localStorage.getItem("formDraft"+userInfo.id)){
        const formData=JSON.parse(localStorage.getItem("formDraft"+userInfo.id));
        if(!formData.id){
          console.log("inside settnig");
          setTitle(formData.title);
          setContent(formData.content);
          setThumbnail(formData.thumbnail);
          setSummary(formData.summary);
          setThumbnailFile(formData.thumbnailFile);
        }
      }
    }
  }, [navigate]);
  ...
  useEffect(() => {
    if(token){
      if(title || summary || content || thumbnail || thumbnailFile){
        if(!isReset){
          setIsReset(true);
        }
        draftRef.current = {
    ...
    }, [title, summary, content, thumbnail, thumbnailFile]);
    ...
    const resetForm = () => {
      ...
      if(token){
        localStorage.removeItem("formDraft"+userInfo.id);
      }
      setIsSubmitLoading(false);
    };
    
    useEffect(() => {
      console.log("ArticleEditor mounted");
      
      const handleBeforeUnload = (event) => {
        saveDraft(); // Save draft on refresh or tab close
      ...
      return () => {
        console.log("ArticleEditor unmounted");
        if(!id){
          saveDraft(); // Save draft on component unmount
        }
    ...
    const saveDraft = () => {
      if(draftRef.current){
        const formData=draftRef.current;
        console.log("Saving draft:", formData);
        localStorage.setItem("formDraft"+userInfo.id, JSON.stringify(formData));
    ...
    const handleContentChange = (value) => {
      if (value === "<p><br></p>") {
        setContent(null); // Or setContent("") based on your preference
      } else {
        setContent(value);
      }
    };
